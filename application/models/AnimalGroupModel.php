...
<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class AnimalGroupModel extends MY_Model {
        private $tableName = 'enem_animals_group';
        ...
        function getDataAnimalGroup ($filter = NULL, $filter_key = NULL, $limit = NULL, $field_target = NULL) {
            if(!empty($filter) && !empty($filter_key)) {
                if($filter === 'id') {
                    if(is_array($limit)) {
                        $sql    =   "SELECT * FROM {$this->tableName} WHERE animals_group_id='".$filter_key."' LIMIT ".$limit['startLimit'].",".$limit['limitData']."";
                    } else {
                        $sql    =   "SELECT * FROM {$this->tableName} WHERE animals_group_id='".$filter_key."'";
                    }
        ...
        function deleteDataAnimalGroup ($field_name, $field_value) {
